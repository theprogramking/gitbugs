      ...
      >
        <div className="h-full overflow-y-auto sidebar-scroll p-4 space-y-4">
          ...
          */}
          ...
          {/* Repositories Filter */}
          <div className="sidebar-section">
            <h3 className="font-semibold mb-3 text-sm sm:text-base">Repositories</h3>
            <div className="space-y-2 max-h-48 overflow-y-auto sidebar-scroll">
              {availableRepositories.slice(0, 15).map((repo) => (
                <label
                  key={repo.name}
                  className="flex items-center gap-2 cursor-pointer text-xs sm:text-sm hover:bg-gray-50 dark:hover:bg-gray-800 p-2 rounded-md transition-colors"
                >
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setSelectedRepositories([...selectedRepositories, repo.name])
                      } else {
                        setSelectedRepositories(selectedRepositories.filter((r) => r !== repo.name))
                      }
                    }}
                    className="github-checkbox"
                  />
                  <span className="truncate flex-1">{repo.name}</span>
                  <span style={{ color: "var(--github-fg-muted)" }}>({repo.count})</span>
                </label>
              ))}
            </div>
          </div>
